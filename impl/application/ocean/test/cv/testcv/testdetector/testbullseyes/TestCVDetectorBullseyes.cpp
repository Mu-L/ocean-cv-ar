...
#include "application/ocean/test/cv/testcv/testdetector/testbullseyes/TestCVDetectorBullseyes.h"
...
#endif
{
	...
	{
		std::cout << "Ocean Framework test for the CV Detector Bullseyes library:" << std::endl << std::endl;
	...
	Log::info() << "Ocean Framework test for the Computer Vision Detector Bullseyes library:";
	Log::info() << " ";
	...
	try
	{
		if (Test::TestCV::TestDetector::TestBullseyes::testCVDetectorBullseyes(testDuration, worker, functionList))
		{
			resultValue = 0;
		}
	}
