...
#include "application/ocean/test/cv/testcv/testdetector/testqrcodes/TestCVDetectorQRCodes.h"
...
#endif
{
	...
	std::string mediaFilename = frameworkPath + std::string("/res/application/ocean/test/cv/testcv/testdetectorqrcodes/sift800x640.bmp");
	...
	{
		std::cout << "Ocean Framework test for the CV Detector QR Codes library:" << std::endl << std::endl;
	...
	try
	{
		if (Test::TestCV::TestDetector::TestQRCodes::testCVDetectorQRCodes(testDuration, worker, functionList))
		{
			resultValue = 0;
		}
	}
