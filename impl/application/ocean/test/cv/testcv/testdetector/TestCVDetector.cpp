...
#include "application/ocean/test/cv/testcv/testdetector/TestCVDetector.h"
...
#endif
{
	...
	commandArguments.registerParameter("image", "i", "The default test image for all tests, e.g., 'sift.png'", Value(mediaFilename));
	...
	{
		std::cout << "Ocean Framework test for the CV Detector library:" << std::endl << std::endl;
	...
	try
	{
		if (Test::TestCV::TestDetector::testCVDetector(testDuration, worker, mediaFilename, functionList))
		{
			resultValue = 0;
		}
	}
