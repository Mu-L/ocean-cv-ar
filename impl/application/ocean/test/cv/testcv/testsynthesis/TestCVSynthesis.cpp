...
#include "application/ocean/test/cv/testcv/testsynthesis/TestCVSynthesis.h"
...
#endif
{
	...
	commandArguments.registerParameter("functions", "f", "The optional subset of functions to test, e.g., \"mappingi\"");
	...
	{
		std::cout << "Ocean Framework test for the CV library:" << std::endl << std::endl;
	...
	Log::info() << "Ocean Framework test for the Computer Vision Synthesis library:";
	Log::info() << " ";
	...
	try
	{
		if (Test::TestCV::TestSynthesis::testCVSynthesis(testDuration, worker, testWidth, testHeight, functionList))
		{
			resultValue = 0;
		}
	}
