...
#include "application/ocean/test/devices/testdevices/TestDevices.h"
...
#endif
{
	...
	commandArguments.registerParameter("functions", "f", "The optional subset of functions to test, e.g., \"gpstracker\"");
	...
	{
		std::cout << "Ocean Framework test for the Devices library:" << std::endl << std::endl;
	...
	try
	{
		if (Test::TestDevices::testDevices(testDuration, worker, functionList))
		{
			resultValue = 0;
		}
	}
