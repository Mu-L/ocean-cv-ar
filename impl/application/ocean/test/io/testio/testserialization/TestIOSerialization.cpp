...
#include "application/ocean/test/io/testio/testserialization/TestIOSerialization.h"
...
#endif
{
	...
	commandArguments.registerParameter("functions", "f", "The optional subset of functions to test, e.g., \"mediaserializer\"");
	...
	{
		std::cout << "Ocean Framework test for the IO Serialization library:" << std::endl << std::endl;
	...
	try
	{
		if (Test::TestIO::TestSerialization::testIOSerialization(testDuration, worker, functionList))
		{
			resultValue = 0;
		}
	}
