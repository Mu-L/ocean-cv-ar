...
bool TestMediaFoundation::test(const double testDuration, const TestSelector& selector)
{
	ocean_assert(testDuration > 0.0);

	TestResult testResult("MediaFoundation test");
	Log::info() << " ";
...
bool TestMediaFoundation::testVideoEncoderDecoder(const double testDuration)
{
	ocean_assert(testDuration > 0.0);
	...
	do
	{
		...
		for (const std::string& mimeType : mimeTypes)
		{
			for (const int iFrameInterval : iFrameIntervals)
			{
				...
				Media::MediaFoundation::VideoEncoder videoEncoder;
				...
				Media::MediaFoundation::VideoDecoder videoDecoder;

				constexpr unsigned int numberFrames = 10u;

				Frames testFrames;
				Media::MediaFoundation::VideoEncoder::Samples encodedSamples;
...

				{
					...
					for (unsigned int frameIndex = 0u; frameIndex < numberFrames; ++frameIndex)
					{
						...
						Media::MediaFoundation::VideoEncoder::Sample sample = videoEncoder.popSample();
				...
				for (const Media::MediaFoundation::VideoEncoder::Sample& sample : encodedSamples)
				{
...

				{
					...
					for (const Media::MediaFoundation::VideoEncoder::Sample& sample : encodedSamples)
					{
						...
						// Try to get any immediately available output
					...
					// Signal end of stream to flush remaining frames from decoder
					videoDecoder.drain();
...
Frame TestMediaFoundation::createTestFrame(const unsigned int width, const unsigned int height, const unsigned int frameIndex)
{
...
bool TestMediaFoundation::validateTestFrame(const Frame& testFrame, const Frame& decodedFrame)
{
